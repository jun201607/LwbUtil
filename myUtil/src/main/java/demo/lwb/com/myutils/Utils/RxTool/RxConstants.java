...
public class RxConstants {
    ...
    //默认保存下载文件目录
    public static final String DOWNLOAD_DIR = FileUtils.getRootPath() + File.separator + "Download" + File.separator + AppUtils.getAppPackageName() + File.separator;

    //图片缓存路径
    public static final String PICTURE_CACHE_PATH = FileUtils.getCacheFolder(RxTool.getContext()) + File.separator + "Picture" + File.separator + "Cache" + File.separator;

    //图片原始路径
    public static final String PICTURE_ORIGIN_PATH = FileUtils.getRootPath() + File.separator + AppUtils.getAppPackageName() + File.separator + "Picture" + File.separator+ "Origin" + File.separator;

    //图片压缩路径
    public static final String PICTURE_COMPRESS_PATH = FileUtils.getRootPath() + File.separator + AppUtils.getAppPackageName() + File.separator + "Picture" + File.separator+ "Compress" + File.separator;

    //默认导出文件目录
    public static final String EXPORT_FILE_PATH = FileUtils.getRootPath() + File.separator + AppUtils.getAppPackageName() + File.separator + "ExportFile" + File.separator;

    //图片名称
    public static final String getPictureName() {
        return TimeUtil.getCurrentDateTime(DATE_FORMAT_LINK) + "_" + new Random().nextInt(1000) + ".jpg";
    }
