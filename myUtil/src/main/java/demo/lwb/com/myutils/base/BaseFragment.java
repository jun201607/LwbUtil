package demo.lwb.com.myutils.base;/*Copyright ©2015 TommyLemon(https://github.com/TommyLemon)
...
public abstract class BaseFragment extends Fragment implements IPopBackStackListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(mView==null){
			...
			if(arguments!=null){
				String title = (String) arguments.get(HomeTab1Presenter.HomeTab1Detail);
	...
	public void showProgressDialog(final int stringResId){
		activity.showProgressDialog(activity.getResources().getString(stringResId));
	}
	/**展示加载进度条,无标题
	 * @param dialogMessage
	 */
	public void showProgressDialog(final String dialogMessage){
		activity.showProgressDialog(dialogMessage);
	}
	/**展示加载进度条
	...
	public void showProgressDialog(final String dialogTitle, final String dialogMessage){
		activity.showProgressDialog(dialogTitle, dialogMessage);
	}
	...
	public void showShortToast(final int stringResId) {
		activity.showShortToast(stringResId);
	}
	/**快捷显示short toast方法，需要long toast就用 Toast.makeText(string, Toast.LENGTH_LONG).show(); ---不常用所以这个类里不写
	 * @param string
	 */
	public void showShortToast(final String string) {
		activity.showShortToast(string);
	}
	/**快捷显示short toast方法，需要long toast就用 Toast.makeText(string, Toast.LENGTH_LONG).show(); ---不常用所以这个类里不写
	 * @param string
	...
	public void showShortToast(final String string, final boolean isForceDismissProgressDialog) {
		activity.showShortToast(string, isForceDismissProgressDialog);
	}
	//show short toast 方法>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	...
	public Handler runThread(final String threadName, final Runnable runnable) {
		return activity.runThread(threadName, runnable);
	}
